<?php
	...
	class Admin_blog_category extends MY_Controller 
	{
		...
		public function load_data_blog_category($limitStart)
		{
		    
			$data['blogs_cat']= $this->admin_blog_category_model->get_all_blog_category($limitStart);
			$this->load->view('admin/load_data_blog_category',$data);
		}
		...
		public function view_form_blog_category($id=0)
		{
		    if($this->session->userdata('id'))
			{
			  ...
			  if($id)
			  {
			     $blog=$this->admin_blog_category_model->get_blog_category($id);
				 $data['id']=$blog->id;
				 $data['name']=$blog->name;
				 
				 
			  }
			  $this->load->view('admin/blog_category_form',$data);
			}
		...
		{
		  $q=$this->admin_blog_category_model->submit_form();
		...
		{
		  $q=$this->admin_blog_category_model->edit_form($id);
		  redirect('admin_blog_category');
		...
		public function remove_blog_category()
		{
		  ...
		  $q=$this->admin_blog_category_model->remove_blog_category();
		  //redirect('admin_blog_category');
		}
