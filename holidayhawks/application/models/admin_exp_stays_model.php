...
*/
error_reporting(0);
class admin_exp_stays_model extends CI_Model
{
   ...
   public function get($limitStart)
   {
	 ...
	 $q = $this->db->query("select * from tbl_exp_stays order by node_order asc limit $limitStart, 30");
	 if($q->num_rows()>0)
	 {
	   return $q->result();
	 }
   ...
   {
     return $query = $this->db->get_where('tbl_pkg_text', array('type'=>'exp_stays'))->row();
   }
   
   public function edit_pkg_text_form()
   {
       
	   $d['exp_desc']=mysql_real_escape_string($this->input->post('descr')).trim();
       $this->db->where('type', 'exp_stays');
   ...
   {
     return $query = $this->db->get_where('tbl_home_text', array('type'=>'exp_stays'))->row();
   }
   ...
   public function remove()
   {
	 ...
	 $this->db->query("delete from tbl_exp_stays where id='$id'"); 
   ...
   {
    return $query = $this->db->get_where('tbl_exp_stays', array('id'=>$id))->row();
   }
   ...
   public function submit_form()
   {
	   ...
	   // $title=mysql_real_escape_string($this->input->post('title').trim());
		...
		$is_home=mysql_real_escape_string($this->input->post('is_home'));
		$q=$this->db->query("select MAX(node_order) as max_order from tbl_exp_stays ")->row();
		...
		if($_FILES['image']['error'] == UPLOAD_ERR_OK)
		{
			...
			/*if($file_size > 1048576){
			 ...
			 if(empty($errors)==true){
				...
				$this->resize_crop_image($wid, $hgh, $big_path, $thumb_path);
		  ...
		  if($img_cover&&$img_cover1&&$img_cover2)
		  {
			  ...
			  $this->db->insert('tbl_exp_stays',$data);
		} 
		 ...
		 if(empty($errors)==true){
		   ...
		   if($_FILES['image_banner1']['error'] == UPLOAD_ERR_OK)
		  {
		   ...
		   $data1 = array(
		     ...
		     $this->db->where('id', $id);
			 $this->db->update('tbl_exp_stays',$data1);
