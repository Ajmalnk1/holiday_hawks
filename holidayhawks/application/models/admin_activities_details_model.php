...
*/
error_reporting(0);
class admin_activities_details_model extends CI_Model
{
   ...
   public function get($limitStart)
   {
	 ...
	 $q = $this->db->query("select a.*,b.name as cat_name from tbl_activities_detail a left join tbl_activities b on a.act_cat_id=b.id order by node_order asc limit $limitStart, 30");
	 if($q->num_rows()>0)
	 {
	   return $q->result();
	 }
   ...
   public function  get_act_detail_about($act_id)
   {
     $query = $this->db->get_where('tbl_activities_about',array('act_id'=>$act_id));
	 if($query->num_rows()>0)
	 {
	   $query=$query->row();
	   return $query->about;
   ...
   public function  get_act_detail_info($act_id)
   {
     $query = $this->db->get_where('tbl_activities_info',array('act_id'=>$act_id));
	 if($query->num_rows()>0)
	 {
   ...
   public function  get_act_detail_gallery($act_id)
   {
     $query = $this->db->get_where('tbl_activities_gallery',array('act_id'=>$act_id));
	 if($query->num_rows()>0)
	 {
	   return $query=$query->result();
   ...
   public function remove()
   {
	 ...
	 $this->db->query("delete from tbl_activities_detail where id='$id'"); 
  ...
  public function remove_pjt_img()
  {
     ...
     $id=$this->input->post('id');
	 $q = $this->db->query("select * from tbl_activities_about_images where id='$id'")->row();
	 $name=$q->image;
	 //echo "delete from tbl_exp_stays_about_images where id='$id'";
	 $this->db->query("delete from tbl_activities_about_images where id='$id'"); 
	 unlink($name);
  ...
  public function remove_pjt_img_gallery()
  {
     ...
     $id=$this->input->post('id');
	 $q = $this->db->query("select * from  tbl_activities_gallery where id='$id'")->row();
	 $name=$q->image;
	 //echo "delete from tbl_exp_stays_about_images where id='$id'";
	 $this->db->query("delete from  tbl_activities_gallery where id='$id'"); 
	 unlink($name);
		 
  } 
  public function act_cats()
  {
    
	$this->db->order_by('name', 'asc');
	return $query = $this->db->get('tbl_activities')->result();
   }
  ...
  public function save_about()
  {
	...
	$this->db->query("delete from tbl_activities_about where act_id='$act_id'"); 
	$data=array('act_id'=>$act_id,'act_cat_id'=>$act_cat_id,'about'=>$desc);
	$this->db->insert('tbl_activities_about',$data);
  ...
  {
    $act_id=$this->input->post('act_id');
	$this->db->query("update tbl_activities_gallery set status=1 where act_id='$act_id'");
  } 
  ...
  public function save_amn()
  {
	 ...
	 $this->db->delete('tbl_activities_avail');
	 $data=array('act_id'=>$act_id,'act_cat_id'=>$act_cat_id,'avail'=>$ami);
	 $this->db->insert('tbl_activities_avail',$data);
  ...
  public function save_info()
  {
	 ...
	 echo $info_title4=$this->input->post('info_title4');
	echo  $info_desc4=$this->input->post('info_desc4');
	 $this->db->where(array('act_id'=>$act_id));
	 $this->db->delete('tbl_activities_info');
	 
	 $data=array('act_id'=>$act_id,'act_cat_id'=>$act_cat_id,'title'=>$info_title,'descr'=>$info_desc);
	 $this->db->insert('tbl_activities_info',$data);
	 if($info_title2!='')
	 {
	   $data2=array('act_id'=>$act_id,'act_cat_id'=>$act_cat_id,'title'=>$info_title2,'descr'=>$info_desc2);
	   $this->db->insert('tbl_activities_info',$data2);
	 }
	 if($info_title3!='')
	 {
	   $data3=array('act_id'=>$act_id,'act_cat_id'=>$act_cat_id,'title'=>$info_title3,'descr'=>$info_desc3);
	   $this->db->insert('tbl_activities_info',$data3);
	 }
	 if($info_title4!='')
	 {
	   $data4=array('act_id'=>$act_id,'act_cat_id'=>$act_cat_id,'title'=>$info_title4,'descr'=>$info_desc4);
	   $this->db->insert('tbl_activities_info',$data4);
	 }
  } 
  ...
  {
    return $query = $this->db->get_where('tbl_activities_detail', array('id'=>$id))->row();
  }
  public function get_id_desti($cat_id)
  {
    $q=$this->db->query("SELECT * FROM `tbl_destination` WHERE id in (select distinct destination_id from tbl_destination_activities where activities_id='$cat_id')");
	if($q->num_rows()>0)
	   {
	     return $q->result();
	   }
  }
  public function get_act_cat_id($id)
  {
    return $q=$this->db->query("select a.*,b.name from  tbl_activities_detail a left join tbl_activities b on a.act_cat_id=b.id where a.id='$id'")->row();
  }
  public function get_desti()
  {
   ...
   public function submit_form()
   {
	   ...
	   // $title=mysql_real_escape_string($this->input->post('title').trim());
		...
		$trial_type=mysql_real_escape_string($this->input->post('trial_type'));
		...
		$min_age=mysql_real_escape_string($this->input->post('min_age'));
		$hand_picked=$this->input->post('hand_picked');
		
		$date=date('Y-m-d H:i:s');
		$q=$this->db->query("select MAX(node_order) as max_order from tbl_activities_detail")->row();
	   ...
	   //$image=$_FILES['image']['name'];
		...
		if($_FILES['image']['error'] == UPLOAD_ERR_OK)
		{
		  ...
		  if($img_cover&&$img_cover1)
		  {
			  ...
			  $this->db->insert('tbl_activities_detail',$data);
        ...
        if($_FILES['image']['error'] == UPLOAD_ERR_OK)
	    {
			...
			/*if($file_size > 1048576){
			 ...
			 if(empty($errors)==true){
				...
				$this->db->where('id', $id);
				$this->db->update('tbl_activities_detail',$data);
		    }
		   ...
		   if($_FILES['image_banner1']['error'] == UPLOAD_ERR_OK)
		  {
		    ...
		    $data1 = array(
		     ...
		     $this->db->where('id', $id);
			 $this->db->update('tbl_activities_detail',$data1);
